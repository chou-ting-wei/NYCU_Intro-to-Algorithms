...
// bool cmp(Pt &a, Pt &b){
//     if(ori(maxcor, a.cor, b.cor) != 0) return ori(maxcor, a.cor, b.cor) > 0;
//     // cerr << a.idx << " " << b.idx << " " << abs2(a.cor - maxcor) << " " << abs2(b.cor - maxcor) << endl;
//     return abs2(a.cor - maxcor) < abs2(b.cor - maxcor);
// }

bool cmp(Pt &a, Pt &b){
    if(a.cor.X != b.cor.X) return a.cor.X < b.cor.X;
    return a.cor.Y < b.cor.Y;
}
...
int32_t main(){
    ...
    For(i, 1, n){
        cin >> pt[i].cor.X >> pt[i].cor.Y;
        pt[i].idx = i;
    }

    sort(pt+1, pt+n+1, cmp);

    vector<Pt> hull(1, pt[1]);
    For(ct, 0, 1){
        int t = sz(hull);
        For(i, 2, n){
            while(sz(hull) > t && ori(hull[sz(hull)-2].cor, hull.back().cor, pt[i].cor) <= 0){
                hull.pop_back();                
            }
            hull.eb(pt[i]);
        }
